...
- **Index `AttributesByClass` by class name** in `Binder.Resolve` (**done**): `GlobalContext` also indexes the class attributes by class name and the parent class attributes by parent class and element type, so `Binder.Resolve` no longer iterates over all class attributes for each class of an element. Only UGGRoyale and Worldly use `mj-class`, so `Render_Templates` does not show it. The new `Render_MjClass` benchmark (`Scenarios/MjClass.mjml`, 40 classes, 240 usages, nested parent class attributes) showed -46.0% / -53.6% time (1.94 ms -> 1.05 ms / 1.89 ms -> 0.88 ms) and -1.9% allocations. Identical output.
- **Validator:** Rendering the templates with `SoftValidator` reports `'mj-class' is not a valid attribute` for every `mj-class` usage (also before these changes). This is a bug of the validator, not a performance issue.
